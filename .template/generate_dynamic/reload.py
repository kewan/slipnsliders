...
def run_command(build, args):
	...
	if args[0] == 'list':
		...
		if len(args) != 3:
			raise ReloadError("Invalid number of arguments passed to 'forge reload pushcdn'")
		pushcdn_stream(build, args[1], args[2])
	else:
...
def push_stream(build, stream_id):
	...
	with lib.temp_file() as zip_file_path:
		...
		with open(zip_file_path, 'rb') as zip_file:
			created = remote._api_post('reload/snapshots/%s/%s' % (build.config['uuid'], stream_id), files={
...
def pushcdn_stream(build, stream_id, manifest_url):
	from forge import build_config
	external = dict()
	external["manifest_url"] = manifest_url
	created = remote._api_post('reload/snapshots/%s/%s' % (build.config['uuid'], stream_id), files={
			'config': StringIO(json.dumps(build_config.load_app())),
			'external': StringIO(json.dumps(external)) 
			})
	if created['result'] != 'ok':
		raise ReloadError("Remote snapshot creation failed")

	LOG.info("Pushed snapshot to stream '%s' with manifest url '%s'" % (stream_id, manifest_url))
